...
namespace Chaira.MesaServicio.Api.Controllers.Soporte
{
    ...
    public class CasoController : ControllerBase
    {
        ...
        public async Task<ActionResult<ApiResponseDto<IEnumerable<CasoDto>>>> GetByTecnico(long idTecnico)
        {
            ...
            return Ok(ApiResponseDto<IEnumerable<CasoDto>>.SuccessResponse(items, "Casos del tÃ©cnico obtenidos"));
        }
        ...
        public async Task<ActionResult<ApiResponseDto<dynamic>>> CrearConSP([FromBody] CasoCreateDto dto)
        {
            try
            {
                if (!ModelState.IsValid)
                    return BadRequest(ApiResponseDto<dynamic>.FailResponse("Datos invÃ¡lidos"));
        ...
        public async Task<ActionResult<ApiResponseDto<dynamic>>> AsignarTecnico(long id, [FromBody] CasoAsignarTecnicoDto dto)
        {
            try
            {
                ...
                return Ok(ApiResponseDto<dynamic>.SuccessResponse(caso, "TÃ©cnico asignado exitosamente"));
            }
            catch (Exception ex)
            {
                return BadRequest(ApiResponseDto<dynamic>.FailResponse($"Error al asignar tÃ©cnico: {ex.Message}"));
            }
        }
            ...
            [FromQuery] string orderDirection = "DESC")
        {
            try
            {
                ...
                return Ok(ApiResponseDto<dynamic>.SuccessResponse(result, "BÃºsqueda completada"));
            }
            catch (Exception ex)
            {
                return BadRequest(ApiResponseDto<dynamic>.FailResponse($"Error en bÃºsqueda: {ex.Message}"));
            }
        }
